...
if __name__ == '__main__':
    ...
    output_cycle = itertools.cycle(itertools.chain.from_iterable(
        [itertools.repeat(p, block_size) for p in bgzip_procs]))
    ...
    for i, p in enumerate(bgzip_procs):
        ...
        p.wait()
        if p.returncode != 0:
            raise Exception("BZGIP process for file {0} failed".format(
                base + '_' + str(i) + ".vcf.gz"))
