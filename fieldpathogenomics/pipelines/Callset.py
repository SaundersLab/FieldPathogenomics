...
class GenomeMask(CheckTargetNonEmpty, SlurmExecutableTask):
    '''Takes the gene annotations and produces a
       BED file of exon regions so we only call
       genotypes in expresses regions '''
    ...
    def output(self):
        return LocalTarget(os.path.join(os.path.dirname(self.gff), "exons_mask.bed"))
    ...
    def work_script(self):
        return '''#!/bin/bash
                  ...
                  gff2bed --do-not-sort < {input} |
                  grep exon |
                  bedtools sort |
                  bedtools merge > {output}.temp

                  mv {output}.temp {output}
        '''.format(input=self.gff,
...
@inherits(GenotypeGVCF, GenomeMask)
class VcfToolsFilter(SlurmExecutableTask, CheckTargetNonEmpty):
    ...
    def requires(self):
        return [self.clone(GenotypeGVCF), self.clone(GenomeMask)]
...
class SampleCoverageFilter(SlurmTask):
    ...
    def work(self):
        ...
        for s, c in zip(callset['samples'], missing):
            print(s.astype('U') + "\t" + str(c))

        with self.output().open('w') as fout:
            fout.write('\n'.join(filtered.astype('U')))
...
if __name__ == '__main__':
    ...
    luigi.run(['CleanUpCallset', '--output-prefix', name,
                                 '--lib-list', json.dumps(lib_list)] + sys.argv[3:])
